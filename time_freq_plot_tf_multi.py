...
for run in ['run_1', 'run_2', 'run_3', 'run_4']:
    ...
    epochs_x_list.append(meg.epochs.copy().pick(x_chs))
    epochs_y_list.append(meg.epochs.copy().pick(y_chs))
    epochs_z_list.append(meg.epochs.copy().pick(z_chs))

del meg

# Concatenate the epochs
epochs_x = concatenate_epochs(epochs_x_list)
epochs_y = concatenate_epochs(epochs_y_list)
epochs_z = concatenate_epochs(epochs_z_list)
...
def plot_tf_matrices(cond, tf, time, freqs, cmap, dB=False):
    ...
    plt.savefig(
        os.path.join(
            'img',
            f'tf_{cond.lower()[:3]}-all_runs{fig_suffix}-{config["axis"]}.png',
        )
    )
    plt.show()
